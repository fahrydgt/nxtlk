...
class Items extends CI_Controller {
        ...
        function view_search($datas=''){
            $data['search_list'] = $this->Items_model->search_result();
            $data['form_setup'] = $this->input->get();
            $data['item_category_list'] = get_dropdown_data(ITEM_CAT,'category_name','id','Items Category'); 
...
//                                );
            ...
            $data['supplier_inv_desc'] = array(
                                        ...
                                        'supplier_item_desc' => $inputs['item_name'],
                                        'purchasing_unit' => $inputs['stock_unit'],
                                        'purchasing_unit_uom_id' => $inputs['item_uom_id'],
                                        'secondary_unit_uom_id' => $inputs['item_uom_id_2'],
                                        'secondary_unit' => $inputs['stock_unit_2'],
            ...
            if($inputs['id']>0 && isset($inputs['prices']['sales']) && !empty($inputs['prices']['sales'])){
                foreach ($inputs['prices']['sales'] as $sales_price){
                    ...
                    $sp_data = array(
                                    ...
                                    'currency_code' =>$sales_price['currency_id'],
            ...
            if($inputs['id']>0 && isset($inputs['prices']['purchasing']) && !empty($inputs['prices']['purchasing'])){
                $purchase_price = $inputs['prices']['purchasing'];
                    $pp_data = array(
                                    ...
                                    'currency_code' =>$purchase_price['purchasing_currency'],
...
//            echo '<pre>';            print_r($data); die;
		...
		$data_view['search_list'] = $this->Items_model->search_result($search_data);
